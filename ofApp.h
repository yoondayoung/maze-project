...
class ofApp : public ofBaseApp {
	...
	public:
		...
		int HEIGHT;//미로의 높이
		int WIDTH;//미로의 너비
		vector<string> input;//텍스트 파일의 모든 정보를 담는 이차원 배열이다.
		vector<vector<int>> visited;//방문여부를 저장할 포인
		int maze_col;//미로칸의 열의 인덱스를 가리킨다.
		int maze_row;//미로칸의 행의 인덱스를 가리킨다.
		int target_x, target_y; // 미로 도착점
		vector<pos> dfs_path; // dfs 미로 경로 저장
		vector<pos> dfs_fail_path; // dfs 미로 실패 경로 저장
		vector<pos> bfs_path; // bfs 미로 경로 저장
		vector<pos> bfs_fail_path; // bfs 미로 실패 경로 저장
